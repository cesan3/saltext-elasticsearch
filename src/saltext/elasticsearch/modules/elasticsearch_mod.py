...
# Defining the classes here because LazyLoader gave me troubles to import them from external files
# This is temporary until I can figure out the imports
...
class ESModule6:
    ...
    def index_create(
        ...
        src_map = None
        if source:
            src_map = __salt__["cp.get_file_str"](
...
def _get_esmodule_class(es_version):
    """
    Gets the ESModule class
    """
    if es_version >= 8:
        return ESModule8
...
if HAS_ELASTICSEARCH:
    ES_MAJOR_VERSION = int(elasticsearch.__version__[0])
    logging.getLogger("elasticsearch").setLevel(logging.CRITICAL)
    profile = __salt__.get("config.option", {}).get("elasticsearch")
    ES_CLS = _get_esmodule_class(ES_MAJOR_VERSION)
    es_instance = ES_CLS(elasticsearch)
    es_instance.set_profile(profile)
...
def __getattr__(name):
    ...
    if es_instance is not None:
        return getattr(es_instance, name)
